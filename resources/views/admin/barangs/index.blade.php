<x-app-layout>
    <!-- Header -->
    @include('layouts.navigation', ['currentPage' => 'List Barang/Produk'])
                            ...
                            <tbody>
                                @foreach ($barangs as $barang)
                                    <tr class="border-b border-gray-200 hover:bg-gray-50">
                                        <td class="p-3">{{ $barang->name }}</td>
                                        <td class="p-3">Rp. {{ number_format($barang->price, 2) }}</td>
                                        <td class="p-3">
                                            @if ($barang->image)
                                                <img src="{{ asset('storage/' . $barang->image) }}"
                                                    alt="{{ $barang->name }}" class="max-w-24">
                                            @else
