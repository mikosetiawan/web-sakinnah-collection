<x-app-layout>
    <!-- Header -->
    @include('layouts.navigation', ['currentPage' => 'Edit Jasa'])
    ...
    <div class="py-12 bg-gray-100">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('jasas.update', $jasa) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="mb-4">
                            ...
                            <input type="text"
                                ...
                                id="name" name="name" value="{{ $jasa->name }}" required>
                        </div>
                        <div class="mb-4">
                            ...
                            <textarea class="block w-full sm:w-96 border-gray-300 rounded-md shadow-sm focus:ring-[#d4af37] focus:border-[#d4af37]"
                                id="description" name="description" rows="4">{{ $jasa->description }}</textarea>
                        </div>
                        <div class="mb-4">
                            ...
                            <input type="number"
                                class="block w-full sm:w-96 border-gray-300 rounded-md shadow-sm focus:ring-[#d4af37] focus:border-[#d4af37]"
                                id="price" name="price" value="{{ $jasa->price }}" step="0.01" required>
                        </div>
                        <div class="mb-4">
                            ...
                            @if ($jasa->image)
                                <img src="{{ asset('storage/' . $jasa->image) }}" alt="{{ $jasa->name }}"
                                    class="mt-2 max-w-24">
                            @endif
                        </div>
                        <button type="submit"
